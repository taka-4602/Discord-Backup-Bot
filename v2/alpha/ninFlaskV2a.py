...
def index():
    try:
        ...
        try:
          serveridj = open(f"{ipath2}{server}.json")
        except:
          f = open(f"{ipath2}{server}.json","w")
          f.write('{}')
          f.close()
          serveridj = open(f"{ipath2}{server}.json")
        ...
        ck=Discord.Utilities.add_role(guild=server, user_id=user, role=serverid["1"])
        if not serverid["1"] in ck["roles"]:
        ...
        if not token in serverid.values():
          ...
          json.dump(serverid, open(f"{ipath2}{server}.json","w"))     
